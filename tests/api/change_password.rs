...
async fn new_password_fields_must_match() {
    ...
    // Act - Part 1 - Try to change the password, but new passwords don't match
...
async fn current_password_must_be_valid() {
    ...
    // Act - Part 1 - Try to change the password, but current password is wrong
    app.post_login(&serde_json::json!({
        "username": &app.test_user.username,
        "password": &app.test_user.password
    }))
...
async fn new_password_must_be_longer_than_12_characters() {
    ...
    // Act - Part 1 - Try to change the password, but it's too short
    app.post_login(&serde_json::json!({
        "username": &app.test_user.username,
        "password": &app.test_user.password
    }))
    ...
    // Assert
    assert!(html_page.contains(
        "<p><i>The new password is too short, must be at least 12 characters long.</i></p>"
    ));
}
...
async fn new_password_must_be_shorter_than_128_characters() {
    ...
    // Assert
    assert!(html_page.contains(
        "<p><i>The new password is too long, must be less than 128 characters long.</i></p>"
    ));
}
